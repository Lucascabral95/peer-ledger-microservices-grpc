...
import (
	...
	walletserver "github.com/peer-ledger/services/wallet-service/internal/server"
...
func run() error {
	...
	repo, err := repository.NewWalletRepositoryFromSQLDB(dbConn)
	if err != nil {
		return fmt.Errorf("create wallet repository: %w", err)
	}

	walletService, err := walletserver.NewWalletGRPCServer(repo)
	if err != nil {
		return fmt.Errorf("create wallet grpc server: %w", err)
	}
