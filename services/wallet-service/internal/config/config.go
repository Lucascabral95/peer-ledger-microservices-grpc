...
func LoadFromLookup(lookup LookupFunc) (*Config, error) {
	...
	if v, err := getInt(lookup, "WALLET_DB_MAX_OPEN_CONNS", cfg.DBMaxOpenConns); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBMaxOpenConns = v
	}

	if v, err := getInt(lookup, "WALLET_DB_MAX_IDLE_CONNS", cfg.DBMaxIdleConns); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBMaxIdleConns = v
	}

	if v, err := getDuration(lookup, "WALLET_DB_CONN_MAX_LIFETIME", cfg.DBConnMaxLifetime); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBConnMaxLifetime = v
	}

	if v, err := getDuration(lookup, "WALLET_DB_CONN_MAX_IDLE_TIME", cfg.DBConnMaxIdleTime); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBConnMaxIdleTime = v
	}

	if v, err := getDuration(lookup, "WALLET_DB_CONNECT_TIMEOUT", cfg.DBConnectTimeout); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBConnectTimeout = v
	}

	if v, err := getInt(lookup, "WALLET_DB_CONNECT_MAX_RETRIES", cfg.DBConnectMaxRetries); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBConnectMaxRetries = v
	}

	if v, err := getDuration(lookup, "WALLET_DB_CONNECT_INITIAL_BACKOFF", cfg.DBConnectInitialBackoff); err != nil {
		errs = append(errs, err.Error())
	} else {
		cfg.DBConnectInitialBackoff = v
	}

	if v, err := getDuration(lookup, "WALLET_DB_CONNECT_MAX_BACKOFF", cfg.DBConnectMaxBackoff); err != nil {
		errs = append(errs, err.Error())
	} else {
...
func (c *Config) Validate() error {
	...
	if strings.TrimSpace(c.GRPCPort) == "" {
		errs = append(errs, "WALLET_GRPC_PORT cannot be empty")
	}
	if strings.TrimSpace(c.WalletDBDSN) == "" {
		errs = append(errs, "WALLET_DB_DSN cannot be empty")
	}
	if c.DBMaxOpenConns <= 0 {
		errs = append(errs, "WALLET_DB_MAX_OPEN_CONNS must be > 0")
	}
	if c.DBMaxIdleConns < 0 {
		errs = append(errs, "WALLET_DB_MAX_IDLE_CONNS must be >= 0")
	}
	if c.DBMaxIdleConns > c.DBMaxOpenConns {
		errs = append(errs, "WALLET_DB_MAX_IDLE_CONNS cannot be greater than WALLET_DB_MAX_OPEN_CONNS")
	}
	if c.DBConnectTimeout <= 0 {
		errs = append(errs, "WALLET_DB_CONNECT_TIMEOUT must be > 0")
	}
	if c.DBConnectMaxRetries < 0 {
		errs = append(errs, "WALLET_DB_CONNECT_MAX_RETRIES must be >= 0")
	}
	if c.DBConnectInitialBackoff <= 0 {
		errs = append(errs, "WALLET_DB_CONNECT_INITIAL_BACKOFF must be > 0")
	}
	if c.DBConnectMaxBackoff <= 0 {
		errs = append(errs, "WALLET_DB_CONNECT_MAX_BACKOFF must be > 0")
	}
	if c.DBConnectInitialBackoff > c.DBConnectMaxBackoff {
		errs = append(errs, "WALLET_DB_CONNECT_INITIAL_BACKOFF cannot be greater than WALLET_DB_CONNECT_MAX_BACKOFF")
	}
