...
func TestLoadFromLookup_Defaults(t *testing.T) {
	...
	if cfg.GRPCPort != "50051" {
		t.Fatalf("expected default GRPCPort 50051, got %s", cfg.GRPCPort)
	}
...
func TestLoadFromLookup_InvalidValues(t *testing.T) {
	env := map[string]string{
		...
		"DB_CONNECT_TIMEOUT":         "bad-duration",
	...
	msg := err.Error()
	if !strings.Contains(msg, "DB_MAX_OPEN_CONNS must be int") {
		t.Fatalf("expected int parse error, got: %s", msg)
	}
	if !strings.Contains(msg, "DB_CONNECT_TIMEOUT must be duration") {
